...
3. **7 級帳戶權限與職級架構 (7-Level RBAC)**：
   ...
   - ⚠️ **開戶權限**：前端開戶卡片只提供 工作人員／主任／總主任 三級；**籌委會架構 51 人已全部預先開戶**（顧問／主席／執行副主席／副主席／總主任／主任／秘書處管理員，見 `data/isd_2026.json` users + `data/committee_accounts.csv`），冇資料嘅可喺「用戶管理」／開戶表／批量 CSV 補。組別下拉（開戶/登記）不含 顧問團 及 主席及執行副主席 兩組。工作人員（數量多、名單後補）由各組自行開戶。
   - 📇 **部門管理中心 = 10 組**（含 顧問團、主席及執行副主席、秘書處，可查看/統計）；**管理資料組卡片 = 8 張**（部門管理中心除顧問及主席之外）：秘書處、會操及典禮組、主題節目組、品牌推廣組、嘉賓接待組、協調組、服務及發展組、行政組；財務已歸入行政組卡片。秘書處（何家耀、陸詠德，地域受薪職員）為管理員，可跨組／為秘書處開戶。
4. **8 大卡片管理模組**：
