...
4. **8 大卡片管理模組**：
   ...
   - 📦 **8. 物資管理**：地域物資及攤位物資申請（v8.5 起**不設庫存表**——地域物資逾萬種、攤位物資向外判商租用，毋須對照庫存，原「庫存」分頁已移除）；依批核權限頁目前設定的批核組／執行組處理，會操及典禮組不會在未被明確指派時自動取得物資批核權。**攤位物資申請對標「2026 攤位總表」**（分區 A–F＋編號＋負責單位由表選擇、攤位名稱由申請人填寫以製作招牌、枱／椅／帳篷圍布（每攤位最多 3 塊）／電源等標準項目只需填數量，另有運送物資需求及其他需求欄）。
