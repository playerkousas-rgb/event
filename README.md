...
3. **7 級帳戶權限與職級架構 (7-Level RBAC)**：
   ...
   - 📇 **部門管理中心 = 10 組**（含 顧問團、主席及執行副主席、秘書處，可查看/統計）。登入後首頁排序為：**我的組別（標亮）→ 7 張公開卡片 → 其他組別及工作卡片 → 管理工具**。10 個組別均有卡片；「開戶／批核中心／權限管理」只供總主任以上查看並固定放最後。物資／泊車證／口頭報價／財務／**車輛** 均不另設獨立卡片，併入協調組／行政組卡片（**泊車證＝車輛通行證申請**，同一件事）。物資及車輛只由協調組總主任以上批核，不會由會操及典禮組批核。
   ...
   - 📁 **會議卡片新增「會議 Drive」分頁（即時連通）**：直接讀取公開的會議 Drive 資料夾（`1-abBGIs37E_cvrHacd1tG9wgWiDRXnz4`，定義於 `data/isd_2026.json` drive.groups.會議），跟 Drive 內分次會議子資料夾（0 Pre-Meeting、第1次…）排列，並可切換「清單／內嵌」模式。GAS 後端 `listDriveFolder` 以 `UrlFetchApp` 公開讀取，毋須要求使用者 Drive 權限；資料夾及檔案必須設為「知道連結的任何人可查看」。
4. **8 大卡片管理模組**：
