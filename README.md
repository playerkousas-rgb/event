...
3. **7 級帳戶權限與職級架構 (7-Level RBAC)**：
   ...
   - ⚠️ **開戶權限**：前端開戶卡片只提供 工作人員／主任／總主任 三級；副主席及以上職級已由管理員預先開設（見 `data/isd_2026.json` users + `data/executive_accounts.csv`），冇資料嘅可喺「用戶管理」／開戶表／批量 CSV 補。組別下拉（開戶/登記）不含 顧問團 及 主席及執行副主席 兩組。
