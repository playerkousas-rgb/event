...
3. **7 級帳戶權限與職級架構 (7-Level RBAC)**：
   ...
   - ⚠️ **開戶權限**：前端開戶卡片只提供 工作人員／主任／總主任 三級；**籌委會架構 54 人已全部預先開戶**（顧問／主席／執行副主席／副主席／總主任／主任／秘書處管理員 5 人，見 `data/isd_2026.json` users + `data/committee_accounts.csv`），冇資料嘅可喺「用戶管理」／開戶表／批量 CSV 補。組別下拉（開戶/登記）不含 顧問團 及 籌委會 兩組。工作人員（數量多、名單後補）由各組自行開戶。
   - 📇 **部門管理中心 = 10 組**（含 顧問團、籌委會、秘書處，可查看/統計）；**管理資料區 = 13 張卡片**：8 張組卡片（秘書處、會操及典禮組、主題節目組、品牌推廣組、嘉賓接待組、協調組、服務及發展組、行政組，財務已歸入行政組卡片）＋ 5 張管理卡片（會議卡片、批核中心、開戶、權限管理、我的監察）。物資／泊車證／口頭報價／財務／**車輛** 均不另設獨立卡片，併入協調組／行政組卡片（**泊車證＝車輛通行證申請**，同一件事），批核中心可連接。秘書處（譚健祥 Figo、何家耀 Oscar、陸詠德 Fiona、鄧倩姸 Cindy、曾凱瑩 Sandy，地域受薪職員）全為管理員，可跨組／為秘書處開戶。
   - 📢 **公開區（7 張，登入前可看）**：公告及溝通（**已併入日程表／旅團須知／活動主題章**）、申請中心、場地與活動總覽、組織架構與聯絡、危機處理、通告及文件、典禮儀式（**已併入獲獎名單**）。
   ...
   - 📁 **會議卡片新增「會議 Drive」分頁（即時連通）**：直接讀取會議 Drive 資料夾（`1-abBGIs37E_cvrHacd1tG9wgWiDRXnz4`，定義於 `data/isd_2026.json` drive.groups.會議），跟 Drive 內分次會議子資料夾（0 Pre-Meeting、第1次…）排列，點擊即展開睇該次會議檔案（預覽／開啟／下載），Drive 有更新即時可見（GAS 後端 `listDriveFolder` action；無後端時顯示 Drive 內嵌資料夾）。
4. **8 大卡片管理模組**：
