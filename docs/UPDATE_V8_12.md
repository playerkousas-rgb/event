...
**未登入＝活動資訊＋一張訪客身份指引卡＋底部 4 個公開資料按鈕；登入後＝原本設計全部還原。** 登入按鈕全站只有一粒，喺最頂 BAR 右上角。
...
| 2 | 登入後不用顯示身份卡片 | 身份卡片**只喺未登入時顯示**（話俾訪客知：初始帳戶＝中文姓名、密碼 1234、開戶搵所屬組別總主任），登入後即隱藏；卡內登入掣 `#identity-login-btn` 已刪，例子帳號改用 MOCK 名「陳子明」（唔再用真人主席名） |
| 3 | 登入只喺右上角出現 | `index.html` 只餘 `#login-toggle-btn` 一粒登入掣；其餘位置（我的監察頁、膳食監察、泊車申請、申請中心提示、聯絡名單「登入可見」等）改為純文字提示「請按右上角「登入」」，唔再係掣 |
| 4 | 登入前「公開資料」／「登入後解鎖」兩堆全部唔要 | 未登入時 `#public-section`、`#identity-section`、`#management-tools-section`、`#simple-mode-note` 全部 `hidden`，卡片格線清空；「登入後解鎖」標題／字眼已刪（身份卡片除外，佢係訪客指引） |
