...
> 最新版另加入 `Approval_Routing`、`Finance_Expenses`、跨裝置本組確認欄位及永久刪除同步。超管在 APP 刪除申請時會呼叫 `deleteRecord`；正式活動回讀以後台陣列為準，所以在工作表刪除的舊測試紀錄不會再被瀏覽器快取還原。`seedInitialData()` 亦不再建立「周恒晉／對講機」測試申請。
