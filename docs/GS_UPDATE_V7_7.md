...
> 最新版另加入 `Approval_Routing`、`Finance_Expenses`、跨裝置本組確認欄位及永久刪除同步。管理員在 APP 刪除申請時會呼叫 `deleteRecord`；正式活動回讀以後台陣列為準，所以在工作表刪除的舊測試紀錄不會再被瀏覽器快取還原。`seedInitialData()` 亦不再建立「周恒晉／對講機」測試申請。

**要唔要更新 GS？→ 要。** 現時物資／車輛通行證／膳食保留原有工作頁，實際批核及執行改由多選動態路由處理，但舊版 GS 有兩個問題會令資料**存唔到試算表**（只留喺自己部機）：
...
前端亦已配合加入 **跨裝置回讀**：正式活動載入時會由 GAS 一次過拉回 `Meal_Orders`、`Supply_Requests`、`Vehicle_Passes`、`Parking_Requests` 及 `Finance_Expenses`。後台陣列是正式活動的權威來源：工作表刪除的紀錄會同步清除舊瀏覽器快取，不會再次出現。
