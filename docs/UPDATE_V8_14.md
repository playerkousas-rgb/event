...
## v8.14e（2026-08-31，用戶拍板）

| 問題 | 決定 |
|------|------|
| 副主席要唔要可以改執行手冊系列？ | **要** |
| 各組總主任要唔要可以改「攤位總表」？ | **唔要**（維持歸主題節目組） |

改動：`CARD_OWNER_EXTRA_ROLES` 入面 `exec_manual`／`activities`／`documents`／`ceremony`／`crisis`
全部加埋 `vice_chairperson`，即係：

```
執行手冊系列（執行手冊・場地與活動總覽・通告及文件・典禮儀式・危機處理）
  ＝ 行政組（負責組） ＋ 執副以上 ＋ 副主席 ＋ 各組總主任
攤位總表（booth）
  ＝ 主題節目組（負責組） ＋ 行政組（統管）  ← 維持不變，其他組總主任只可看
```
卡面標示原本已寫「行政組・副主席以上・總主任可修改／上傳／更新」，同實際一致。
