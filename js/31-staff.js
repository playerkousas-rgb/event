...
,
  renderStaffContacts(){
    ...
    container.innerHTML=`
      <div class="space-y-4">
        ${!loggedIn?`<div class="bg-amber-50 border border-amber-200 rounded-xl p-3 text-[11px] text-amber-900 flex flex-wrap items-center gap-2 justify-between"><span><i class="fa-solid fa-lock mr-1"></i><b>聯絡資料受保護：</b>姓名、職銜、組別公開可看；電話及 Email 需登入後才顯示。</span><button onclick="app.openLoginModal()" class="bg-sky-600 text-white px-3 py-1.5 rounded-xl text-[11px] font-bold"><i class="fa-solid fa-right-to-bracket mr-1"></i>登入查看聯絡資料</button></div>`:''}
        <div class="flex flex-wrap gap-2">
...
,
  filterStaffContacts(){
    ...
    const phoneCell=(c,cls)=>loggedIn?`<span class="${cls||''}">${escapeHtml(c.contact||'—')}</span>`:`<button onclick="app.openLoginModal()" class="bg-slate-100 hover:bg-slate-200 border text-slate-500 px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap" title="登入後可見"><i class="fa-solid fa-lock mr-0.5"></i>登入可見</button>`;
    const emailCell=(c)=>loggedIn?`<span class="font-mono text-[11px]">${escapeHtml(c.email||'—')}</span>`:`<button onclick="app.openLoginModal()" class="bg-slate-100 hover:bg-slate-200 border text-slate-500 px-2 py-0.5 rounded-full text-[10px] font-bold whitespace-nowrap" title="登入後可見"><i class="fa-solid fa-lock mr-0.5"></i>登入可見</button>`;
