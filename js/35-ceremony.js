...
,
  sortCeremonyResponses(sort){
    ...
    const statusEl=document.getElementById('ceremony-area-status'); if(statusEl){const areas=[...new Set(rows.map(r=>r.area).filter(Boolean))]; statusEl.innerHTML=areas.map(a=>{const rs=rows.filter(r=>r.area===a),done=rs.length>0&&rs.every(r=>r.ticked);return `<span class="inline-flex items-center gap-1 mr-2 mb-1 px-2 py-1 rounded-lg text-[10px] ${done?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}">${escapeHtml(a)} ${rs.filter(r=>r.ticked).length}/${rs.length} ${done?'已確認完成':'進行中'} ${canTick&&!done?`<button onclick="app.confirmCeremonyArea('${escapeHtml(a)}')" class="underline font-bold">確認本區完成</button>`:''}</span>`;}).join('');}
