...
,
  sortCeremonyResponses(sort){
    ...
    const statusEl=document.getElementById('ceremony-area-status'); if(statusEl){const arrived=rows.filter(r=>r.ticked).length, repliedAttend=rows.filter(r=>r.response==='已回覆'&&r.attendance==='出席').length, repliedMissing=rows.filter(r=>r.response==='已回覆'&&r.attendance==='出席'&&!r.ticked).length; const areas=[...new Set(rows.map(r=>r.area).filter(Boolean))]; statusEl.innerHTML=`<div class="text-[11px] font-bold text-slate-700 mb-2">到達／已 TICK：${arrived}/${rows.length}　｜　回覆出席但未見：${repliedMissing}　｜　回覆出席：${repliedAttend}</div>`+areas.map(a=>{const rs=rows.filter(r=>r.area===a),done=rs.length>0&&rs.every(r=>r.ticked);return `<span class="inline-flex items-center gap-1 mr-2 mb-1 px-2 py-1 rounded-lg text-[10px] ${done?'bg-emerald-100 text-emerald-700':'bg-amber-100 text-amber-700'}">${escapeHtml(a)} ${rs.filter(r=>r.ticked).length}/${rs.length} ${done?'全部已 TICK':'進行中'}</span>`;}).join('');}
