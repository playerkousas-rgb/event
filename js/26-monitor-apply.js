...
,
  ...
  renderApplyHubModule(){
    ...
    const items=[
      ...
      {key:'booth', icon:'fa-store', color:'bg-orange-100 text-orange-700', title:'攤位物資申請', desc:'與外判商租用枱／椅／布置等；與地域物資借用分開', badge:canSubmitSupply?'登入可申請':'請先登入', badgeCls:canSubmitSupply?'bg-orange-100 text-orange-700 border-orange-200':'bg-slate-100 text-slate-600 border-slate-200', action:`app.openModule('booth')`, enabled:canSubmitSupply},
